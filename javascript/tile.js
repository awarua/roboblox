...
Tile.prototype.display = function display(x, y) {
  if (paramsChanged){
    this.calculateGroup();
...
Tile.prototype.calculateGroup = function(){
  ...
  for (let i = 0; i < numOpenings; i++){
    ...
    if (path.numEdges > 1){
      paths = this.splitPath(path, path.numEdges);
...
Tile.prototype.calculatePath = function(oIdx, cIdx, colIdx){
...
Tile.prototype.calculateEdge = function(eIdx, colIdx){
  path = new Path('LINE', eIdx, eIdx, colIdx);
...
Tile.prototype.drawToCanvas = function(x, y) { 
  push();
 ...
 */
Tile.prototype.toSVGString = function(){
  let margin = 0;
  let doJoin = true;
...
Tile.prototype.toJSONString = function(){
  let retJSON = [];
  for (let i = 0; i < this.drawingData.length; i++){
 ...
 */
Tile.prototype.toSVGGroup = function(c, r, margin, join){
...
Tile.prototype.avgVertex = function(o, c){
  let oIdx = o % this.sides.length;
  let cIdx = c % this.sides.length;

  oV = this.vertices[oIdx];
  cV = this.vertices[cIdx];
...
Tile.prototype.findNextOpening = function(searchFrom){
...
Tile.prototype.findClosing = function(searchFrom){
  for (let i = 0; i < this.sides.length && this.sides[searchFrom]; i++){
    searchFrom = (searchFrom + 1) % this.sides.length;
  }
...
Tile.prototype.countOpenings = function(){
  let numOpenings = 0;
...
Tile.prototype.getNextWallOpening = function(lastWallOpening){  
...
Tile.prototype.splitPath = function(path, numPieces){
...
Tile.prototype.splitPathDataToPieces = function(v0, pathPart, numHalfPieces){
...
Tile.prototype.splitPathData = function(v0, pathParams, t){
...
Tile.prototype.pathFromBezierData = function(data){
...
Tile.prototype.pathFromLineData = function(data){
...
Tile.prototype.checkCompatability = function checkCompatability(other){
...
Tile.prototype.top = function top(){
  let t = 0;
  if (this.sides[1]){
    t = 10; 
  }
...
Tile.prototype.right = function right(){
  let r = 0;
  if (this.sides[2]){
    r = 10; 
  }
...
Tile.prototype.bottom = function bottom(){
  let b = 0;
  if (this.sides[4]){
    b = 10; 
  }
...
Tile.prototype.left = function left(){
  let l = 0;
  if (this.sides[7]){
    l = 10; 
  }
